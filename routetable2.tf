...
output "rt_id" {
  value = azurerm_route_table.routetablename.*.id
}
...
resource "azurerm_subnet_route_table_association" "subnetassociate" {
  count             = 2
  subnet_id         = element(azurerm_subnet.subnets.*.id,count.index)  
  route_table_id     = element(azurerm_route_table.routetablename.*.id,count.index +5)
  }
*/
resource "azurerm_subnet_route_table_association" "subnetassociate1" {
  count             = 2
  subnet_id         = element(azurerm_subnet.subnets.*.id,count.index)  
  route_table_id     = element(azurerm_route_table.routetablename.*.id,count.index+1)
  }
